...
- Linux 클라이언트
  ...
  - 진입점: `client/linux/install_linux.sh`
- Windows 클라이언트
  - `gpu-agent`, `telegraf`
  - 진입점: `client/windows/install_windows.ps1`
- Kubernetes 클라이언트 클러스터
  - `dcgm-exporter`, `telegraf`, `validator`
  - 진입점: `kubectl apply -k client/k8s`
...
3. `client/linux/install_linux.sh` 실행
...
3. `client/windows/install_windows.ps1` 실행
...
`server/ingest/`는 다음 payload를 받습니다.
...
- [ingest 설명](server/ingest/README.md)
- [K8s 테스트 오버레이 설명](client/k8s/test/README.md)
